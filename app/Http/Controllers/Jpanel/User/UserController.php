...
class UserController extends Controller
{
    ...
    public function updateUser(Request $request,$user_id){
        ...
        $user = User::where('id', $user_id)->update(['name' => $request->uname, 'username' => $request->username, 'phone' => $request->phone, 'email' => $request->email,'updated_by'=>Auth::id()]);
        if ($user) {
    ...
    public function userRole(Request $request){
        ...
        if($request->status=='1'){
            ...
            foreach ($roleModules as $roleModule) {
                ...
                Permission::insert(['user_id' => $request->user_id,'module_id' => $module,'action_id' =>1,'created_by'=>Auth::id()]);//add  create permission in user_permission table
                Permission::insert(['user_id' => $request->user_id,'module_id' => $module,'action_id' =>2,'created_by'=>Auth::id()]);//add  read permission in user_permission table
                Permission::insert(['user_id' => $request->user_id,'module_id' => $module,'action_id' =>3,'created_by'=>Auth::id()]);//add  update permission in user_permission table
                Permission::insert(['user_id' => $request->user_id,'module_id' => $module,'action_id' =>4,'created_by'=>Auth::id()]);//add  delete permission in user_permission table
            }
            ...
            foreach ($roleModules as $roleModule) {
                $module = $roleModule->module_id;
                Permission::where('user_id', $request->user_id)->where('module_id', $module)->update(['deleted_by'=>Auth::id()]); // Delete all permission for role module
    ...
    public function userDelete(Request $request){
        User::where('id',$request->user_id)->update(['deleted_by'=>Auth::id()]);
        User::find($request->user_id)->delete();
        Permission::where('user_id', $request->user_id)->update(['deleted_by'=>Auth::id()]); // Delete all permission of user
    ...
    public function userPermissionsUpdate(Request $request){
        if($request->status=='1'){
                Permission::insert(['user_id' => $request->user_id,'module_id' => $request->module_id,'action_id' =>$request->action_id,'created_by'=>Auth::id()]);//add  create permission in user_permission table
        }else{
                Permission::where('user_id', $request->user_id)->where('module_id', $request->module_id)->where('action_id', $request->action_id)->update(['deleted_by'=>Auth::id()]); // Delete all permission for role module
