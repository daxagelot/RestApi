...
class RoleController extends Controller
{
    ...
    public function updateRole (Request $request,$role_id){
        ...
        $user = Role::where('id', $role_id)->update(['name' => $request->rname, 'slug' => $request->slug,'updated_by'=>Auth::id()]);
        if ($user) {
    ...
    public function roleModule(Request $request){
        ...
        if($request->status=='1'){
            ...
            foreach ($userRoles as $userRole) {
                ...
                Permission::insert(['user_id' => $user_id,'module_id' => $request->module_id,'action_id' =>1,'created_by'=>Auth::id()]);//add  create permission in user_permission table
                Permission::insert(['user_id' => $user_id,'module_id' => $request->module_id,'action_id' =>2,'created_by'=>Auth::id()]);//add  read permission in user_permission table
                Permission::insert(['user_id' => $user_id,'module_id' => $request->module_id,'action_id' =>3,'created_by'=>Auth::id()]);//add  update permission in user_permission table
                Permission::insert(['user_id' => $user_id,'module_id' => $request->module_id,'action_id' =>4,'created_by'=>Auth::id()]);//add  delete permission in user_permission table
            }
        }else{
            ...
            foreach ($userRoles as $userRole) {
                $user_id = $userRole->user_id;
                Permission::where('user_id', $user_id)->where('module_id', $request->module_id)->update(['deleted_by'=>Auth::id()]); // Delete all permission for role module
    ...
    {
        Role::where('id',$request->role_id)->update(['deleted_by'=>Auth::id()]);
